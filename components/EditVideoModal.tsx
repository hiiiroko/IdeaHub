    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative w-full max-w-md bg-white rounded-xl shadow-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold">编辑视频</h2>
          <button onClick={onClose} disabled={working} className="text-gray-500">✕</button>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input value={tagsInput} onChange={e=>setTagsInput(e.target.value)} placeholder="用逗号分隔" className="w-full px-3 py-2 border rounded" />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} disabled={working} className="px-4 py-2 rounded bg-gray-100 text-gray-700 disabled:opacity-50">取消</button>
        ...
        {working && (
          <div className="absolute left-6 bottom-6 flex items-center gap-2 text-sm text-gray-600">
