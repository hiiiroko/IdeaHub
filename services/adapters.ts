...
export const toUiVideo = (v: DbVideo): UiVideo => {
  return {
    ...
    viewCount: v.view_count ?? (v as any).total_views ?? 0,
    likeCount: v.like_count ?? (v as any).total_likes ?? 0,
    commentCount: v.comment_count ?? (v as any).total_comments ?? 0,
    hot_score: typeof v.hot_score === 'number' ? v.hot_score : undefined,
